...
une_result une_type_flt_as_int(une_result result)
{
    ...
    return (une_result){.kind = UNE_RK_INT, .value._int = (une_int)result.value._flt};
}
...
{
    assert(result.kind == UNE_RK_FLT);
    return (une_result){.kind = UNE_RK_STR, .value._wcs = une_flt_to_wcs(result.value._flt)};
}
...
une_result une_type_flt_add(une_result left, une_result right)
{
    ...
    if (right.kind == UNE_RK_INT)
        return (une_result){.kind = UNE_RK_FLT,
                            .value._flt = left.value._flt + (une_flt)right.value._int};
    if (right.kind == UNE_RK_FLT)
        return (une_result){.kind = UNE_RK_FLT, .value._flt = left.value._flt + right.value._flt};
...
une_result une_type_flt_fdiv(une_result left, une_result right)
{
    ...
    if ((right.kind == UNE_RK_INT && right.value._int == 0) ||
        (right.kind == UNE_RK_FLT && une_flts_equal(right.value._flt, UNE_NEW_FLT(0.0))))
        return (une_result){.kind = UNE_RK_FLT, .value._flt = UNE_INFINITY};
    ...
    if (right.kind == UNE_RK_FLT)
        return (une_result){.kind = UNE_RK_INT,
                            .value._int =
                                (une_int)(une_flt_floor(left.value._flt / right.value._flt))};
    return une_result_create(UNE_RK_ERROR);
}
...
{
    assert(result.kind == UNE_RK_FLT);
    return (une_result){.kind = UNE_RK_FLT, .value._flt = -result.value._flt};
}
