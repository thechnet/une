...
{
    assert(result.kind == UNE_RK_INT);
    return (une_result){.kind = UNE_RK_FLT, .value._flt = (une_flt)result.value._int};
}
...
une_result une_type_int_as_str(une_result result)
{
    ...
    return (une_result){.kind = UNE_RK_STR, .value._wcs = out};
}
...
une_result une_type_int_add(une_result left, une_result right)
{
    ...
    if (right.kind == UNE_RK_INT)
        return (une_result){.kind = UNE_RK_INT, .value._int = left.value._int + right.value._int};
...
une_result une_type_int_pow(une_result left, une_result right)
{
    ...
    if (right.kind == UNE_RK_INT) {
        ...
        return (une_result){.kind = UNE_RK_INT,
                            .value._int = (une_int)(une_flt_pow((une_flt)left.value._int,
                                                                (une_flt)right.value._int))};
    }
    if (right.kind == UNE_RK_FLT)
        return (une_result){.kind = UNE_RK_FLT,
...
{
    assert(result.kind == UNE_RK_INT);
    return (une_result){.kind = UNE_RK_INT, .value._int = -result.value._int};
}
