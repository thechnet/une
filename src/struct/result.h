...
*/
#define UNE_FOR_RESULT_LIST_ITEM(var, size) for (size_t var = 1; var <= size; var++)

/*
Iterate over every index in a une_result list.
*/
#define UNE_FOR_RESULT_LIST_INDEX(var, size) for (size_t var = 0; var <= size; var++)

/*
Condition to check whether une_result_kind is valid.
*/
#define UNE_RESULT_KIND_IS_VALID(kind) (kind > UNE_RK_none__ && kind < UNE_RK_max__)
