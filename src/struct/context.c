...
static une_association *une_variable_find_or_create_(une_context *starting_context, wchar_t *name, une_result content, bool global, bool create_if_not_found)
{
	...
	assert(
		(name && content.kind == UNE_RK_none__) ||
	...
	if (!association && create_if_not_found) {
		...
		association->content = une_result_create(UNE_RK_VOID); /* Don't use UNE_RK_none__ because this will be freed using une_result_free. */
...
une_association *une_variable_create(une_context *context, wchar_t *name)
{
	...
	variable->content = une_result_create(UNE_RK_VOID); /* Don't use UNE_RK_none__ because this will be freed using une_result_free. */
...
{
	return une_variable_find_or_create_(context, name, une_result_create(UNE_RK_none__), false, false);
}
...
{
	return une_variable_find_or_create_(context, name, une_result_create(UNE_RK_none__), true, false);
}
...
{
	return une_variable_find_or_create_(context, name, une_result_create(UNE_RK_none__), false, true);
}
...
{
	return une_variable_find_or_create_(context, name, une_result_create(UNE_RK_none__), true, true);
}
