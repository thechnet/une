...
{
    return (une_token){
        .kind = kind, .pos = (une_position){.start = 0, .end = 0}, .value._vp = NULL};
}
...
une_static__ wchar_t *une_token_to_wcs(une_token token)
{
    ...
    assert(UNE_TOKEN_KIND_IS_VALID(token.kind));
    str_len += swprintf(
        str + wcslen(str), UNE_SIZE_TOKEN_AS_WCS, L"%ls", une_token_kind_to_wcs(token.kind));
    ...
    /* Numbers. */
    case UNE_TK_INT:
        str_len += swprintf(str + wcslen(str),
                            UNE_SIZE_TOKEN_AS_WCS,
                            L":" UNE_COLOR_TOKEN_VALUE UNE_PRINTF_UNE_INT UNE_COLOR_RESET,
                            token.value._int);
        break;
    case UNE_TK_FLT: {
        ...
        str_len += swprintf(str + wcslen(str),
                            UNE_SIZE_TOKEN_AS_WCS,
                            L":" UNE_COLOR_TOKEN_VALUE L"%ls" UNE_COLOR_RESET,
                            flt_as_wcs);
        free(flt_as_wcs);
        break;
    }
    ...
    case UNE_TK_STR:
        str_len += swprintf(
            str + wcslen(str),
            UNE_SIZE_TOKEN_AS_WCS,
            L":" UNE_COLOR_TOKEN_VALUE L"\"%ls" UNE_COLOR_TOKEN_VALUE L"\"" UNE_COLOR_RESET,
            token.value._wcs);
        break;
    case UNE_TK_NAME:
        str_len += swprintf(str + wcslen(str),
    ...
    case UNE_TK_NATIVE:
        str_len += swprintf(str + wcslen(str),
                            UNE_SIZE_TOKEN_AS_WCS,
                            L":" UNE_COLOR_TOKEN_VALUE L"%ls" UNE_COLOR_RESET,
                            une_native_to_wcs((une_native)token.value._int));
        break;
