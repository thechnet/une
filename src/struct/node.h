...
*/
#define UNE_NODE_KIND_IS_VALID(kind) (kind > UNE_NK_none__ && kind < UNE_NK_max__)

/*
Condition to check whether une_node_kind is in interpreter lookup table.
*/
#define UNE_NODE_KIND_IS_IN_LUT(kind) (kind >= UNE_R_BGN_LUT_NODES && kind <= UNE_R_END_LUT_NODES)
...
*/
#define UNE_FOR_NODE_LIST_ITEM(var, size) for (size_t var = 1; var <= size; var++)

/*
Iterate over every index in a une_node list.
*/
#define UNE_FOR_NODE_LIST_INDEX(var, size) for (size_t var = 0; var <= size; var++)
