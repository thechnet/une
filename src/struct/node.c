...
const wchar_t *une_node_table[] = {
    ...
    L"MOD",         L"POW",
    L"NEG",         L"SEEK",
    L"IDX_SEEK",    L"MEMBER_SEEK",
    L"ASSIGN",      L"ASSIGNADD",
    L"ASSIGNSUB",   L"ASSIGNPOW",
    L"ASSIGNMUL",   L"ASSIGNFDIV",
    L"ASSIGNDIV",   L"ASSIGNMOD",
    L"CALL",        L"FOR_RANGE",
    L"FOR_ELEMENT", L"WHILE",
    L"IF",          L"ASSERT",
    L"CONTINUE",    L"BREAK",
    L"RETURN",      L"EXIT",
    L"ANY",         L"ALL",
    L"COVER",       L"CONCATENATE",
    L"THIS",        L"OBJECT_ASSOCIATION",
};
...
void une_node_free(une_node *node, bool free_wcs)
{
    ...
    case UNE_NK_NAME:
        ...
        We don't normally free the WCS pointers because they are pointing at data stored in the
        tokens, which may still be needed after the parse. This memory is freed alongside the
        tokens. */
        if (free_wcs)
...
wchar_t *une_node_to_wcs(une_node *node)
{
    ...
    case UNE_NK_STMTS: {
        ...
        for (size_t i = 2; i <= list_size; i++) {
            ...
            offset += swprintf(
                buffer + offset, UNE_SIZE_NODE_AS_WCS, UNE_COLOR_RESET L"\n%ls", node_as_wcs);
    ...
    case UNE_NK_LIST: {
        ...
        if (list_size == 0) {
            buffer_len +=
                swprintf(buffer, UNE_SIZE_NODE_AS_WCS, UNE_COLOR_RESET L"%c%c", open, close);
            break;
        }
        wchar_t *node_as_wcs = une_node_to_wcs(list[1]);
        int offset =
            swprintf(buffer, UNE_SIZE_NODE_AS_WCS, UNE_COLOR_RESET L"%c%ls", open, node_as_wcs);
        free(node_as_wcs);
        for (size_t i = 2; i <= list_size; i++) {
            node_as_wcs = une_node_to_wcs(list[i]);
            offset += swprintf(
                buffer + offset, UNE_SIZE_NODE_AS_WCS, UNE_COLOR_RESET L", %ls", node_as_wcs);
            assert(buffer_len + offset < UNE_SIZE_NODE_AS_WCS);
            free(node_as_wcs);
        }
        buffer_len +=
            offset + swprintf(buffer + offset, UNE_SIZE_NODE_AS_WCS, UNE_COLOR_RESET L"%c", close);
        break;
    }
    ...
    case UNE_NK_ID:
        buffer_len += swprintf(
            buffer,
            UNE_SIZE_NODE_AS_WCS,
            UNE_COLOR_NODE_DATUM_KIND L"%ls" UNE_COLOR_RESET L":" UNE_COLOR_NODE_DATUM_VALUE
                UNE_PRINTF_UNE_INT UNE_COLOR_RESET,
    ...
    case UNE_NK_FLT: {
        ...
        buffer_len += swprintf(
            buffer,
            UNE_SIZE_NODE_AS_WCS,
            UNE_COLOR_NODE_DATUM_KIND L"%l"
                                      L"s" UNE_COLOR_RESET L":" UNE_COLOR_NODE_DATUM_VALUE L"%l"
                                                                                           L"s" UNE_COLOR_RESET,
            une_node_kind_to_wcs(node->kind),
            flt_as_wcs);
        free(flt_as_wcs);
    ...
    case UNE_NK_STR:
        buffer_len += swprintf(
            buffer,
            UNE_SIZE_NODE_AS_WCS,
            UNE_COLOR_NODE_DATUM_KIND L"%ls" UNE_COLOR_RESET L":" UNE_COLOR_NODE_DATUM_VALUE L"\"%"
                                                                                             L"l"
                                                                                             L"s" UNE_COLOR_NODE_DATUM_VALUE L"\"" UNE_COLOR_RESET,
            une_node_kind_to_wcs(node->kind),
        ...
        buffer_len += swprintf(
            ...
            UNE_COLOR_NODE_DATUM_KIND L"%l"
                                      L"s" UNE_COLOR_RESET L":" UNE_COLOR_NODE_DATUM_VALUE L"%l"
                                                                                           L"s" UNE_COLOR_RESET,
            une_node_kind_to_wcs(node->kind),
            une_native_to_wcs((une_native)node->content.value._int));
        break;
    ...
    case UNE_NK_BREAK:
    case UNE_NK_CONTINUE: {
        buffer_len += swprintf(buffer,
                               UNE_SIZE_NODE_AS_WCS,
                               UNE_COLOR_NODE_BRANCH_KIND L"%ls" UNE_COLOR_RESET,
                               une_node_kind_to_wcs(node->kind));
        break;
    }
    ...
    case UNE_NK_EXIT: {
        ...
        buffer_len += swprintf(
            buffer,
            UNE_SIZE_NODE_AS_WCS,
            UNE_COLOR_RESET L"(" UNE_COLOR_NODE_BRANCH_KIND L"%l"
                                                            L"s" UNE_COLOR_RESET L" %"
                                                                                 L"l"
                                                                                 L"s" UNE_COLOR_RESET L")",
            une_node_kind_to_wcs(node->kind),
            branch1);
    ...
    case UNE_NK_FUNCTION: {
        ...
        buffer_len += swprintf(
            ...
            UNE_COLOR_RESET L"(" UNE_COLOR_NODE_BRANCH_KIND L"%l"
                                                            L"s" UNE_COLOR_RESET L" %"
                                                                                 L"l"
                                                                                 L"s" UNE_COLOR_RESET L", %ls" UNE_COLOR_RESET L")",
            une_node_kind_to_wcs(node->kind),
            branch1,
            branch2);
        free(branch1);
    ...
    case UNE_NK_SEEK: {
        ...
        buffer_len +=
            swprintf(buffer,
                     UNE_SIZE_NODE_AS_WCS,
                     UNE_COLOR_RESET L"(" UNE_COLOR_NODE_BRANCH_KIND L"%ls" UNE_COLOR_RESET
                                     " %ls" UNE_COLOR_RESET L", GLOBAL:%d)",
                     une_node_kind_to_wcs(node->kind),
                     branch1,
                     ((bool)node->content.branch.b == true) ? 1 : 0);
        free(branch1);
        break;
    }
    ...
    case UNE_NK_IF: {
        ...
        buffer_len += swprintf(
            ...
            UNE_COLOR_RESET L"(" UNE_COLOR_NODE_BRANCH_KIND L"%l"
                                                            L"s" UNE_COLOR_RESET L" %"
                                                                                 L"l"
                                                                                 L"s" UNE_COLOR_RESET L", %ls" UNE_COLOR_RESET L", %ls" UNE_COLOR_RESET L")",
            une_node_kind_to_wcs(node->kind),
            branch1,
            branch2,
            branch3);
    ...
    case UNE_NK_FOR_RANGE: {
        ...
        buffer_len += swprintf(
            buffer,
            UNE_SIZE_NODE_AS_WCS,
            UNE_COLOR_RESET L"(" UNE_COLOR_NODE_BRANCH_KIND L"%ls" UNE_COLOR_RESET L" %"
                                                                                   L"l"
                                                                                   L"s" UNE_COLOR_RESET L", %ls" UNE_COLOR_RESET L", %ls" UNE_COLOR_RESET L", %ls" UNE_COLOR_RESET L")",
            une_node_kind_to_wcs(node->kind),
            branch1,
            branch2,
            branch3,
