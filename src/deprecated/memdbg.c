...
void *memdbg_malloc(char *file, int line, size_t size)
{
    ...
    if (memory == NULL) {
        memdbg_alert_count++;
        fail_at(
            file, line, MEMDBG_MSG_OUT_OF_MEMORY " (malloc %zu+%zu b)", size, MEMDBG_PADDING_SIZE);
    }
    memdbg_allocation_add((memdbg_allocation){
        .file = file, .line = line, .memory = memory, .size = size, .check_padding = true});
    return memory;
}
...
void *memdbg_realloc(char *file, int line, void *memory, size_t size)
{
    ...
    if (memory_new == NULL) {
        memdbg_alert_count++;
        fail_at(
            file, line, MEMDBG_MSG_OUT_OF_MEMORY " (realloc %zu+%zu b)", size, MEMDBG_PADDING_SIZE);
    }
...
static void memdbg_init(void)
{
    ...
    if (memdbg_allocations == NULL) {
        memdbg_alert_count++;
        fail(MEMDBG_MSG_OUT_OF_MEMORY " (memdbg_init %zu b)",
             memdbg_allocations_size * sizeof(*memdbg_allocations));
    }
    ...
    if (signal(SIGFPE, &memdbg_signal_handler__) == SIG_ERR ||
        signal(SIGILL, &memdbg_signal_handler__) == SIG_ERR ||
        signal(SIGSEGV, &memdbg_signal_handler__) == SIG_ERR) {
        memdbg_alert_count++;
    ...
    if (memdbg_allocations != NULL) {
        ...
        for (size_t i = 0; i < memdbg_allocations_size; i++)
            if (memdbg_allocations[i].memory != NULL) {
                ...
                memdbg_alert_count++;
                warn_at(
                    memdbg_allocations[i].file, memdbg_allocations[i].line, MEMDBG_MSG_NOT_FREED);
            }
...
void memdbg_allocations_padding_check__(char *file, int line)
{
    ...
    for (size_t i = 0; i < memdbg_allocations_size; i++)
        if (memdbg_allocations[i].memory != NULL && memdbg_allocations[i].check_padding)
            if (memcmp(memdbg_allocations[i].memory + memdbg_allocations[i].size,
                       memdbg_padding,
                       MEMDBG_PADDING_SIZE) != 0) {
                memdbg_allocations[i].check_padding = false;
                memdbg_alert_count++;
                if (file == NULL)
                    warn_at(
                        memdbg_allocations[i].file, memdbg_allocations[i].line, MEMDBG_MSG_PADDING);
                else
                    warn_at(memdbg_allocations[i].file,
                            memdbg_allocations[i].line,
                            MEMDBG_MSG_PADDING_AT,
                            file,
                            line);
            }
}
...
static inline memdbg_allocation memdbg_allocation_reset(void)
{
    return (memdbg_allocation){
        .file = NULL, .line = 0, .memory = NULL, .size = 0, .check_padding = true};
}
...
static inline void memdbg_allocation_add(memdbg_allocation allocation)
{
    ...
    if (index == memdbg_allocations_size) {
        ...
        memdbg_allocations = realloc(memdbg_allocations,
                                     (size_t)memdbg_allocations_size * sizeof(*memdbg_allocations));
        if (memdbg_allocations == NULL) {
            memdbg_alert_count++;
            fail(MEMDBG_MSG_OUT_OF_MEMORY " (memdbg_allocation_add %zu b)",
                 memdbg_allocations_size * sizeof(*memdbg_allocations));
        }
...
#define memdbg_wrap_allocator(                                                                     \
    type_, name_, params_, memory_, call_, newsize_, check_incoming_memory_, enable_padding_)      \
    ...
    {                                                                                              \
        ...
        if (enable_padding_) {                                                                     \
            ...
            if (memory_new == NULL) {                                                              \
                memdbg_alert_count++;                                                              \
                fail(MEMDBG_MSG_OUT_OF_MEMORY " (" #name_ " %zu+%zu b)",                           \
                     size,                                                                         \
                     MEMDBG_PADDING_SIZE);                                                         \
            }                                                                                      \
        }                                                                                          \
        memdbg_allocation_add((memdbg_allocation){.memory = (char *)memory_new,                    \
                                                  .size = size,                                    \
                                                  .file = file,                                    \
                                                  .line = line,                                    \
...
memdbg_wrap_allocator(char *,
    ...
    memdbg_wrap_allocator(wchar_t *,
                          wcsdup,
                          (char *file, int line, wchar_t *wcs),
                          wcs,
                          wcsdup(wcs),
                          wcslen(memory_new) + 1,
                          true,
        ...
        memdbg_wrap_allocator(FILE *,
                              fopen,
                              (char *file, int line, char *path, char *mode),
                              path,
                              fopen(path, mode),
                              1,
                              false,
            ...
            memdbg_wrap_deallocator(
                int, fclose, (char *file, int line, FILE *fp), fp, fclose(fp), false)

                memdbg_wrap_function(void *,
                                     memdbg_memset__,
                                     (char *file, int line, void *str, int c, size_t n),
                                     str,
                                     memset(str, c, n))
