...
une_node *une_parse(une_error *error, une_parser_state *ps, une_token *tokens)
{
    ...
    return une_parse_sequence(error,
                              ps,
                              UNE_NK_STMTS,
                              UNE_TK_none__,
                              UNE_TK_NEW,
                              UNE_TK_EOF,
                              &une_parse_directive_or_block);
}
...
une_parser__(une_parse_body)
{
    ...
    if (now(&ps->in).kind == UNE_TK_LBRC)
        ...
        LOGPARSE_END(une_parse_sequence(error,
                                        ps,
                                        UNE_NK_STMTS,
                                        UNE_TK_none__,
                                        UNE_TK_none__,
...
une_parser__(une_parse_and_or)
{
    ...
    LOGPARSE_END(une_parse_binary_operation(error,
                                            ps,
...
une_parser__(une_parse_power)
{
    ...
    LOGPARSE_END(
        une_parse_binary_operation(error,
                                   ps,
                                   UNE_TK_POW,
                                   UNE_NK_POW,
                                   UNE_TK_POW,
                                   &une_parse_accessor,
                                   &une_parse_minus /* We parse a lower precedence here because
                                                       powers are evaluated right to left. */
                                   ));
}
...
une_parser__(une_parse_str)
{
    ...
    while (now(&ps->in).kind == UNE_TK_STR_EXPRESSION_BEGIN) {
        ...
        string_with_expression->pos =
            une_position_between(expression->pos, post_expression_string->pos);
        string_with_expression->pos =
            une_position_between(expression->pos, post_expression_string->pos);
...
une_parser__(une_parse_seek, bool global)
{
    ...
    if (!name) {
        *error = une_error_create(); /* une_parse_name normally returns an error, but here we want
                                        to ignore it. */
        LOGPARSE_END(NULL);
    }
...
une_parser__(une_parse_list)
{
    ...
    LOGPARSE_END(une_parse_sequence(
        error, ps, UNE_NK_LIST, UNE_TK_LSQB, UNE_TK_SEP, UNE_TK_RSQB, &une_parse_expression));
}
...
une_parser__(une_parse_signature)
{
    ...
    if (now(&ps->in).kind == UNE_TK_LPAR)
        parameters = une_parse_sequence(
            error, ps, UNE_NK_LIST, UNE_TK_LPAR, UNE_TK_SEP, UNE_TK_RPAR, &une_parse_name);
    else
        parameters = une_parse_sequence(
            error, ps, UNE_NK_LIST, UNE_TK_none__, UNE_TK_none__, UNE_TK_none__, &une_parse_name);
    if (!parameters)
        LOGPARSE_END(NULL);
...
une_parser__(une_parse_return)
{
    ...
    une_position pos = now(&ps->in).pos; /* If the parser finds a return
                                                                                        value after
                                            this, pos.end is changed again further down. */
    pull(&ps->in); /* Return. */

    /* Return Value. */
    une_node *value = NULL; /* DOC: NULL here means no return value was specified.
                                                         This tells the interpreter that there is no
                               return value. */
    if (now(&ps->in).kind != UNE_TK_NEW && now(&ps->in).kind != UNE_TK_EOF &&
        now(&ps->in).kind != UNE_TK_RBRC) {
...
une_parser__(une_parse_exit)
{
    ...
    une_position pos = now(&ps->in).pos; /* If the parser finds an exit
                                                                                    code after this,
                                            pos.end is changed again further down. */
    pull(&ps->in); /* Exit. */
...
une_parser__(une_parse_assignment_or_expr_stmt)
{
    ...
    ptrdiff_t before_assignee = une_parser_checkpoint(
        ps); /* Needed to backstep in case this is not a variable assignment. */
    une_node *assignee = une_parse_assignee(error, ps);
    une_node_kind assignment_operation = UNE_NK_none__;
    if (assignee && now(&ps->in).kind >= UNE_R_BGN_ASSIGNMENT_TOKENS &&
        now(&ps->in).kind <= UNE_R_END_ASSIGNMENT_TOKENS) {
        assignment_operation =
            UNE_R_BGN_ASSIGNMENT_NODES + (now(&ps->in).kind - UNE_R_BGN_ASSIGNMENT_TOKENS);
        pull(&ps->in);
    } else {
    ...
    if (assignment_operation) {
        ...
        assignment_or_expression_statement->pos =
            une_position_between(assignee->pos, expression->pos);
...
une_parser__(une_parse_index)
{
    ...
    une_node *index =
        une_node_create(UNE_NK_none__); /* The caller is required to provide the node kind. */
    index->pos.end =
        peek(&ps->in, -1).pos.end; /* The caller is required to provide the start position. */
...
une_parser__(une_parse_call)
{
    ...
    /* Arguments. */
    une_node *arguments = une_parse_sequence(
        error, ps, UNE_NK_LIST, UNE_TK_LPAR, UNE_TK_SEP, UNE_TK_RPAR, &une_parse_expression);
    if (!arguments)
        LOGPARSE_END(NULL);
...
une_parser__(
    ...
    une_token_kind
        end, /* If omitted, the sequence stops immediately after one element has been parsed. */
    une_node *(*parser)(une_error *, une_parser_state *))
{
    ...
    while (true) {
        ...
        /* If no end was provided, stop the sequence here. It's important that we leave subsequent
        whitespace intact, because those tokens might be needed as delimiters in encompassing
        sequences. */
