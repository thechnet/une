...
void une_traceback_print_trace(une_context *context)
{
    ...
    fwprintf(UNE_ERROR_STREAM,
             UNE_COLOR_TRACEBACK_LOCATION L"File \"%hs\", line %zu",
             module_name,
             position.line);
    if (context->label)
    ...
    fwprintf(UNE_ERROR_STREAM,
             UNE_COLOR_HINT L"    Position: %zu-%zu" UNE_COLOR_RESET L"\n",
             position.start,
             position.end);
#endif
    ...
    while (index < position.end) {
        ...
        fwprintf(UNE_ERROR_STREAM,
                 UNE_COLOR_RESET UNE_TRACEBACK_EXTRACT_PREFIX L"%.*ls\n",
                 length_of_line,
                 module->source + index);

        size_t underline_offset = position.start > index ? position.start - index : 0;
        size_t underline_length =
            (position.end < index + length_of_line ? position.end - index : length_of_line) -
            underline_offset;
        if (index + underline_offset + underline_length ==
                position.end - 1 && /* Underline ends on this line. */
            (module->source[position.end - 1] == L'\n' ||
             module->source[position.end - 1] ==
                 L'\0') /* Position ends on a newline or the end of the source. */
        )
            underline_length++;
