...
une_interpreter__(*interpreter_table__[]) = {
    &une_interpret_void,        &une_interpret_int,         &une_interpret_flt,
    &une_interpret_str,         &une_interpret_list,        &une_interpret_object,
    &une_interpret_function,    &une_interpret_native,      &une_interpret_stmts,
    &une_interpret_cop,         &une_interpret_not,         &une_interpret_and,
    &une_interpret_or,          &une_interpret_nullish,     &une_interpret_equ,
    &une_interpret_neq,         &une_interpret_geq,         &une_interpret_gtr,
    &une_interpret_leq,         &une_interpret_lss,         &une_interpret_add,
    &une_interpret_sub,         &une_interpret_mul,         &une_interpret_fdiv,
    &une_interpret_div,         &une_interpret_mod,         &une_interpret_pow,
    &une_interpret_neg,         &une_interpret_seek,        &une_interpret_idx_seek,
    &une_interpret_member_seek, &une_interpret_assign,      &une_interpret_assign_add,
    &une_interpret_assign_sub,  &une_interpret_assign_pow,  &une_interpret_assign_mul,
    &une_interpret_assign_fdiv, &une_interpret_assign_div,  &une_interpret_assign_mod,
    &une_interpret_call,        &une_interpret_for_range,   &une_interpret_for_element,
    &une_interpret_while,       &une_interpret_if,          &une_interpret_assert,
    &une_interpret_continue,    &une_interpret_break,       &une_interpret_return,
    &une_interpret_exit,        &une_interpret_any,         &une_interpret_all,
    &une_interpret_cover,       &une_interpret_concatenate, &une_interpret_this,
};
...
{
    return (une_result){.kind = UNE_RK_VOID, .value._int = node->content.value._int};
}

une_interpreter__(une_interpret_int)
{
    return (une_result){.kind = UNE_RK_INT, .value._int = node->content.value._int};
}

une_interpreter__(une_interpret_flt)
{
    return (une_result){.kind = UNE_RK_FLT, .value._flt = node->content.value._flt};
}

une_interpreter__(une_interpret_str)
{
    /* DOC: Memory Management: Here we can see that results DUPLICATE strings. */
    une_result result = {.kind = UNE_RK_STR, .value._wcs = wcsdup(node->content.value._wcs)};
...
une_interpreter__(une_interpret_object)
{
    ...
    return (une_result){.kind = UNE_RK_OBJECT, .value._vp = (void *)object};
}
...
une_interpreter__(une_interpret_function)
{
    ...
    return (une_result){.kind = UNE_RK_FUNCTION, .value._id = callable->id};
}

une_interpreter__(une_interpret_native)
{
    return (une_result){.kind = UNE_RK_NATIVE, .value._int = node->content.value._int};
}
...
une_interpreter__(une_interpret_stmts)
{
    ...
    UNE_FOR_NODE_LIST_ITEM(i, nodes_size)
    {
        ...
        /* Break if required. */
        if (result_.kind == UNE_RK_ERROR || result_.kind == UNE_RK_CONTINUE ||
            result_.kind == UNE_RK_BREAK || felix->is.should_return || felix->is.should_exit)
            break;
    }
...
une_interpreter__(une_interpret_not)
{
    ...
    return (une_result){.kind = UNE_RK_INT, .value._int = !is_true};
}
...
une_interpreter__(une_interpret_member_seek)
{
    ...
    if (subject.kind == UNE_RK_OBJECT) {
        ...
        subject = (une_result){
            ...
            .reference = (une_reference){.kind = UNE_FK_SINGLE, .root = (void *)object_container}};
    }
...
une_interpreter__(une_interpret_assign)
{
    ...
    /* Check if value can be assigned. If .can_assign is undefined, assume any value can be
     * assigned. */
    ...
    if (assignee_type.assign) {
        ...
        *root = value; /* Instead of copying this result, we just use the original; this way, we
                          also don't need to worry about freeing it. */
    }
    une_result_free(assignee);
...
une_interpreter__(une_interpret_assign_add)
{
    ...
    *subject = result; /* Instead of copying this result, we just use the original; this way, we
                          also don't need to worry about freeing it. */
...
une_interpreter__(une_interpret_call)
{
    ...
    if (is_method_call) {
        ...
        assert(felix->is.this_contestant.kind == UNE_RK_REFERENCE ||
               felix->is.this_contestant.kind == UNE_RK_OBJECT);
...
une_interpreter__(une_interpret_for_range)
{
    ...
    une_association *var = une_variable_find_by_name_or_create(
        felix->is.context, name); /* We only check the *local* variables. */

    /* Loop. */
    for (une_int i = from; i != till; i += step) {
        var = une_variable_find_by_name(felix->is.context,
                                        name); /* Avoid stale pointer if variable buffer grows. */
        une_result_free(var->content);
        var->content = (une_result){.kind = UNE_RK_INT, .value._int = i};
...
une_interpreter__(une_interpret_return)
{
    ...
    if (node->content.branch.a != NULL)
        result =
            une_interpret(node->content.branch.a); /* Dereferencing happens in interpret_stmts. */
    else
...
une_interpreter__(une_interpret_seek_or_create, bool existing_only)
{
    ...
    return (une_result){.kind = UNE_RK_REFERENCE,
                        .reference =
                            (une_reference){.kind = UNE_FK_SINGLE, .root = (void *)&var->content}};
}
...
une_interpreter__(une_interpret_idx_seek_index)
{
    ...
    /* If the subject was NOT a reference (i.e. we interpreted a literal), we need to dereference
     * the retrieved data *now*, because the literal will be deleted upon completion of this
     * function. */
...
une_interpreter__(une_interpret_comparison, une_int (*comparator)(une_result, une_result))
{
    ...
    une_result left = une_result_dereference(
        une_interpret(une_node_unwrap_any_or_all(node->content.branch.a, &left_wrapped_as)));
    if (left.kind == UNE_RK_ERROR)
        return left;
    ...
    une_result right = une_result_dereference(
        une_interpret(une_node_unwrap_any_or_all(node->content.branch.b, &right_wrapped_as)));
    if (right.kind == UNE_RK_ERROR) {
        une_result_free(left);
        return right;
    }
    ...
    return (une_result){.kind = UNE_RK_INT, .value._int = applies};
}
