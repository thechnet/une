...
une_range une_range_from_relative_index(une_result index, size_t scope)
{
    ...
    return (une_range){.valid = valid, .first = (size_t)absolute_index};
}
...
{
    return (une_position){.start = first.start, .end = last.end, .line = first.line};
}
