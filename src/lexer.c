...
void une_lex(une_error *error, une_lexer_state *ls)
{
    ...
    while (true) {
        ...
        if (ls->begin_str_expression) {
            ...
            une_lexer_commit(ls,
                             (une_token){.kind = UNE_TK_STR_EXPRESSION_BEGIN,
                                         .pos = (une_position){.start = ls->text_index - 1,
                                                               .end = ls->text_index,
                                                               .line = ls->line},
                                         .value._vp = 0});
        }
        ...
        if (une_lexer_now(ls) == L'\0') {
            if (ls->tokens_count > 0 && ls->tokens[ls->tokens_count - 1].kind != UNE_TK_NEW)
                une_lexer_commit(ls,
                                 (une_token){.kind = UNE_TK_NEW,
                                             .pos = (une_position){.start = ls->text_index,
                                                                   .end = ls->text_index + 1,
                                                                   .line = ls->line},
                                             .value._vp = NULL});
            une_lexer_commit(ls,
                             (une_token){.kind = UNE_TK_EOF,
                                         .pos = (une_position){.start = ls->text_index,
                                                               .end = ls->text_index + 1,
                                                               .line = ls->line},
                                         .value._vp = NULL});
        ...
        if (ls->in_str_expression && une_lexer_now(ls) == L'}') {
            une_lexer_commit(ls,
                             (une_token){.kind = UNE_TK_STR_EXPRESSION_END,
                                         .pos = (une_position){.start = ls->text_index,
                                                               .end = ls->text_index + 1,
                                                               .line = ls->line},
                                         .value._vp = 0});
        ...
        if (UNE_LEXER_WC_IS_HARD_WHITESPACE(une_lexer_now(ls))) {
            ...
            if (ls->tokens_count > 0 && ls->tokens[ls->tokens_count - 1].kind != UNE_TK_NEW)
                une_lexer_commit(ls,
                                 (une_token){.kind = UNE_TK_NEW,
                                             .pos = (une_position){.start = idx_left,
                                                                   .end = ls->text_index,
                                                                   .line = ls->line},
                                             .value._vp = 0});
            ls->line += lines;
        ...
        *error = UNE_ERROR_SET(
            UNE_EK_SYNTAX,
            ((une_position){.start = ls->text_index, .end = ls->text_index + 1, .line = ls->line}));
...
{
    for (une_token_kind kind = UNE_R_BGN_OPERATOR_TOKENS; kind <= UNE_R_END_OPERATOR_TOKENS;
         kind++) {
        ptrdiff_t i = 0;
        while (une_lexer_peek(ls, i) == une_token_table[kind - 1][i]) {
            if (une_token_table[kind - 1][++i] == L'\0') {
                ...
                for (; i > 0; i--)
                    une_lexer_advance(ls);
                return (une_token){.kind = kind,
                                   .pos = (une_position){.start = (size_t)starting_index,
                                                         .end = ls->text_index,
                                                         .line = ls->line}};
            }
        }
...
une_lexer__(une_lex_number, bool allow_signed)
{
    ...
    une_token number = {
        ...
        .pos = (une_position){.start = start_index, .end = ls->text_index, .line = ls->line}};
    if (is_floating)
...
une_lexer__(une_lex_string)
{
    ...
    while (true) {
        ...
        if (une_lexer_now(ls) == L'\0') {
            *error = UNE_ERROR_SET(UNE_EK_SYNTAX,
                                   ((une_position){.start = ls->text_index,
                                                   .end = ls->text_index + 1,
                                                   .line = ls->line}));
            free(buffer);
            return une_token_create(UNE_TK_none__);
    ...
    return (une_token){
        .kind = UNE_TK_STR,
        .pos = (une_position){.start = idx_start, .end = ls->text_index, .line = line_start},
        /* DOC: Memory Management: This is where strings referenced during tokenization and parsing
           are constructed. */
        .value._wcs = buffer,
    };
}
...
keyword_or_name_defined:
    if (tk.kind != UNE_TK_NAME)
        free(buffer);
    tk.pos = (une_position){.start = idx_start, .end = ls->text_index, .line = ls->line};
...
bool une_lex_number_integer(une_error *error,
                            une_lexer_state *ls,
                            int base,
                            une_int *integer,
                            bool allow_signed,
                            ...
                            size_t limit_length)
{
    ...
    *integer = 0;
    ...
    while (true) {
        if (!une_lexer_digit_to_decimal(une_lexer_now(ls), &digit_in_decimal) ||
...
bool une_lex_number_fractional_part(une_error *error,
                                    une_lexer_state *ls,
                                    int base,
                                    une_flt *floating)
{
