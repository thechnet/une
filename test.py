...
# Options
SKIP_UNTIL = int(argv[1]) if len(
    argv) > 1 and argv[1] != "enumerate_cases" else 0  # 0, 2
HIDE_OUTPUT = True
SHOW_ERRORS = False
CLEAR = False
STOP_AT_FAIL = True
...
cases = [
    ...
    Case('input("Write \'test\' and press enter: ")',
         UNE_RK_STR, 'test', [ATTR_STDIN]),
    ...
    Case('write("script.une", "msg=128");script("script.une");return msg',
         UNE_RK_INT, '128', [ATTR_NO_IMPLICIT_RETURN]),

    Case('write("script.une", "4");append("script.une", "\n6");return read("script.une")',
    ...
    Case('replace("a","","xax");return replace("a","","xax")',
         UNE_RK_STR, 'xx', [ATTR_NO_IMPLICIT_RETURN]),
    ...
    Case('({a:46,b:()->return this.a,c:n->{this.a=n;return this}}).c(128).b()',
         UNE_RK_INT, '128', []),
    Case('a={b:0,c:()->return this.b,d:n->this.b=n};a.d(46);return a.c()',
         UNE_RK_INT, '46', [ATTR_NO_IMPLICIT_RETURN]),
    Case('main={attr:0,with_set_attr:value->{this.attr=value;return this}};return main.with_set_attr(({value:23,double:()->return this.value*2}).double()).attr;',
         UNE_RK_INT, '46', [ATTR_NO_IMPLICIT_RETURN]),
    Case('c=[{o:{a:0,s:n->this.a=n}}];c[0].o.s(46);return c[0].o.a',
         UNE_RK_INT, '46', [ATTR_NO_IMPLICIT_RETURN]),
    Case('o={a:0,m:v->{this.a=v;return this}};o.m(1).m(2);return o.a',
         UNE_RK_INT, '2', [ATTR_NO_IMPLICIT_RETURN]),  # Method chaining
    Case('a=()->return{b:()->return a()};a()', UNE_RK_OBJECT, '{b: <function>}',
         # Ensure member_seek receives object references
         [ATTR_NO_IMPLICIT_RETURN]),
    ...
    Case('global a=46;return a', UNE_RK_INT,
         '46', [ATTR_NO_IMPLICIT_RETURN]),
    Case('a=()->{global b=46};a();return b',
         UNE_RK_INT, '46', [ATTR_NO_IMPLICIT_RETURN]),
    Case('a=[1]**2', UNE_RK_ERROR, UNE_EK_TYPE, [ATTR_NO_IMPLICIT_RETURN]),
    Case('a=a', UNE_RK_ERROR, UNE_EK_SYMBOL_NOT_DEFINED,
         [ATTR_NO_IMPLICIT_RETURN]),

    # SET_IDX
    Case('a=[0];a[0]=1;return a', UNE_RK_LIST,
         '[1]', [ATTR_NO_IMPLICIT_RETURN]),
    Case('a=[0];global a[0]=1;return a', UNE_RK_LIST,
         '[1]', [ATTR_NO_IMPLICIT_RETURN]),
    Case('a=[[0]];a[0][0]=46;a', UNE_RK_LIST,
         "[[46]]", [ATTR_NO_IMPLICIT_RETURN]),
    Case('a=[0];a[0]=[1]**2', UNE_RK_ERROR,
         UNE_EK_TYPE, [ATTR_NO_IMPLICIT_RETURN]),
    Case('a[0]=1', UNE_RK_ERROR, UNE_EK_SYMBOL_NOT_DEFINED,
         [ATTR_NO_IMPLICIT_RETURN]),
    Case('a=1;a[0]=1', UNE_RK_ERROR,
         UNE_EK_TYPE, [ATTR_NO_IMPLICIT_RETURN]),
    Case('a=[0];a[[1]]=1', UNE_RK_ERROR,
         UNE_EK_INDEX, [ATTR_NO_IMPLICIT_RETURN]),
    Case('a=[0];a[1]=1', UNE_RK_ERROR,
         UNE_EK_INDEX, [ATTR_NO_IMPLICIT_RETURN]),

    # Assignment operations.
    Case('a=23;a+=23;return a', UNE_RK_INT,
         '46', [ATTR_NO_IMPLICIT_RETURN]),
    Case('a=23;a-=23;return a', UNE_RK_INT,
         '0', [ATTR_NO_IMPLICIT_RETURN]),
    Case('a=4;a**=2;return a', UNE_RK_INT,
         '16', [ATTR_NO_IMPLICIT_RETURN]),
    Case('a=4;a*=2;return a', UNE_RK_INT, '8', [ATTR_NO_IMPLICIT_RETURN]),
    Case('a=5;a//=2;return a', UNE_RK_INT, '2', [ATTR_NO_IMPLICIT_RETURN]),
    Case('a=5;a/=2;return a', UNE_RK_FLT,
         flt('2.5'), [ATTR_NO_IMPLICIT_RETURN]),
    ...
    # Set member.
    Case('a={b:0};a.b=46;return a.b', UNE_RK_INT,
         '46', [ATTR_NO_IMPLICIT_RETURN]),

    # FOR
    Case('a=0;for i from 0 till 3 a=a+i;return a',
         UNE_RK_INT, '3', [ATTR_NO_IMPLICIT_RETURN]),
    Case('a=0;for i from 3 till 0{if i==2 continue;a=a+i};return a',
         UNE_RK_INT, '4', [ATTR_NO_IMPLICIT_RETURN]),
    Case('a=0;for i from 0 till 0 a=1;return a',
         UNE_RK_INT, '0', [ATTR_NO_IMPLICIT_RETURN]),
    Case('a=0;for i from 0 till 1{break;a=1};return a', UNE_RK_INT, '0', [
        ATTR_NO_IMPLICIT_RETURN]),
    Case('for i from [1]**2 till 3 print(i)',
         UNE_RK_ERROR, UNE_EK_TYPE, []),
    Case('for i from [1] till 3 print(i)', UNE_RK_ERROR, UNE_EK_TYPE, []),
    Case('for i from 0 till [1]**2 print(i)',
         UNE_RK_ERROR, UNE_EK_TYPE, []),
    Case('for i from 0 till 3 print([i]**2)',
         UNE_RK_ERROR, UNE_EK_TYPE, []),
    Case('for i in 0 print(i)', UNE_RK_ERROR, UNE_EK_TYPE, []),
    Case('a=0;for i in [1,2,3] a=a+i;return a',
         UNE_RK_INT, '6', [ATTR_NO_IMPLICIT_RETURN]),

    # WHILE
    Case('i=3;while i>0 i=i-1;return i', UNE_RK_INT,
         '0', [ATTR_NO_IMPLICIT_RETURN]),
    Case('i=0;while i>0{i=-1;break};return i',
         UNE_RK_INT, '0', [ATTR_NO_IMPLICIT_RETURN]),
    ...
    Case('if 0 return 0;return -1', UNE_RK_INT,
         '-1', [ATTR_NO_IMPLICIT_RETURN]),
    Case('if 1 return 1 elif 0 return 0 else return 0',
         UNE_RK_INT, '1', [ATTR_NO_IMPLICIT_RETURN]),
    Case('if 0 return 0 elif 2 return 2 else return 0',
         UNE_RK_INT, '2', [ATTR_NO_IMPLICIT_RETURN]),
    Case('if 0 return 0 elif 0 return 0 else return 3',
         UNE_RK_INT, '3', [ATTR_NO_IMPLICIT_RETURN]),
    Case('if 0 return 0 elif 0 return 0;return',
         UNE_RK_VOID, 'Void', [ATTR_NO_IMPLICIT_RETURN]),
    Case('if [1]**2 1', UNE_RK_ERROR, UNE_EK_TYPE, []),

    # CALLABLES
    Case('fn=arg->{a=[0];a[0]=1;for i from 0 till 2{if i==0 continue;break};return [arg*1*1.1, "str"]}',
         UNE_RK_VOID, 'Void', [ATTR_NO_IMPLICIT_RETURN]),
    Case('a=()->{return};a=()->{return}', UNE_RK_VOID,
         'Void', [ATTR_NO_IMPLICIT_RETURN]),
    Case('int=()->{return}', UNE_RK_ERROR, UNE_EK_SYNTAX, []),

    # CALL
    Case('fn=arg->{a=[0];a[0]=1;for i from 0 till 2{if i==0 continue;break};return [arg*1*1.1, "str"]};return fn(2)',
         UNE_RK_LIST, '[2.2, "str"]', [ATTR_NO_IMPLICIT_RETURN]),
    Case('a()', UNE_RK_ERROR, UNE_EK_SYMBOL_NOT_DEFINED, []),
    Case('a=b->return b;c=d->return a();c(1)',
         UNE_RK_ERROR, UNE_EK_CALLABLE_ARG_COUNT, []),
    Case('a=(b,c)->{return b};a(1, [1]**2)',
         UNE_RK_ERROR, UNE_EK_TYPE, []),
    Case('1()', UNE_RK_ERROR, UNE_EK_TYPE, []),

    # TYPES
    Case('print(int)', UNE_RK_VOID, 'Void', []),
    Case('if int{return 1}else{return 0}',
         UNE_RK_INT, '1', [ATTR_NO_IMPLICIT_RETURN]),
    Case('print(()->{})', UNE_RK_VOID, 'Void', []),
    Case('if ()->{}{return 1}else{return 0}',
         UNE_RK_INT, '1', [ATTR_NO_IMPLICIT_RETURN]),
    Case('if print(1){return 1}else{return 0}',
         UNE_RK_INT, '0', [ATTR_NO_IMPLICIT_RETURN]),

    # ERROR DISPLAY
    Case('1\n2\n3/0\n4', UNE_RK_ERROR,
         UNE_EK_ZERO_DIVISION, [ATTR_FILE_ONLY]),

    # ORDER OF OPERATIONS
    Case('-2**2', UNE_RK_INT, '-4', []),
    Case('a=[2];return -a[0]**2', UNE_RK_INT,
         '-4', [ATTR_NO_IMPLICIT_RETURN]),
    Case('a=()->return 2;return -a()**2', UNE_RK_INT,
         '-4', [ATTR_NO_IMPLICIT_RETURN]),
    ...
    Case(
        '()->{return ()->{return [[()->{return 46}]]}}()()[0][0]()', UNE_RK_INT, '46', []),

    # STALE POINTER IN FOR LOOP IMPLEMENTATION
    Case('a=0;for i from 1 till 3 for j from 4 till 6 a=a+i*j;return a',
         UNE_RK_INT, '27', [ATTR_NO_IMPLICIT_RETURN]),

    # LAST RESULT ALWAYS RETURNED
    Case('46', UNE_RK_INT, '46', [ATTR_NO_IMPLICIT_RETURN]),

    # RETURN WITHOUT RETURN VALUE
    Case('return;return 46', UNE_RK_VOID,
         'Void', [ATTR_NO_IMPLICIT_RETURN]),

    # EXIT
    Case('exit;return 0', UNE_RK_VOID, 'Void', [ATTR_NO_IMPLICIT_RETURN]),
    Case('exit 46;return 0', UNE_RK_INT, '46', [ATTR_NO_IMPLICIT_RETURN]),
    Case('exit "string"', UNE_RK_ERROR,
         UNE_EK_TYPE, [ATTR_NO_IMPLICIT_RETURN]),
    Case('()->{exit(46)}();return 0', UNE_RK_INT,
         '46', [ATTR_NO_IMPLICIT_RETURN]),

    # Slices
    Case('a="";a="b"+"c"', UNE_RK_VOID, 'Void', [ATTR_NO_IMPLICIT_RETURN]),
    Case('[1, 2, 3, 4, 5][1..-1][1..Void]', UNE_RK_LIST, '[3, 4]', []),
    Case('a=[1, 2, 3, 4, 5];return a[1..-1][1..Void]',
         UNE_RK_LIST, '[3, 4]', [ATTR_NO_IMPLICIT_RETURN]),
    Case('a=[1, 2, 3, 4, 5];a[1..-1][1..Void]=[23, 46];return a',
         UNE_RK_LIST, '[1, 2, 23, 46, 5]', [ATTR_NO_IMPLICIT_RETURN]),
    Case('"string"[1..-1][1..Void]', UNE_RK_STR, 'rin', []),
    Case('a="string";return a[1..-1][1..Void]',
         UNE_RK_STR, 'rin', [ATTR_NO_IMPLICIT_RETURN]),
    Case('a="string";a[1..-1][1..Void]="4t6";return a',
         UNE_RK_STR, 'st4t6g', [ATTR_NO_IMPLICIT_RETURN]),
    ...
    Case('assert False', UNE_RK_ERROR,
         UNE_EK_ASSERTION_NOT_MET, [ATTR_NO_IMPLICIT_RETURN]),

    # Don't propagate 'return' out of eval() and script().
    Case('eval("return");return 1', UNE_RK_INT,
         '1', [ATTR_NO_IMPLICIT_RETURN]),

    # sort
    Case('sort([2,1,3],(a,b)->return a-b)', UNE_RK_LIST, '[1, 2, 3]', []),
    Case('sort([2,1,3],(a,b)->return b-a)', UNE_RK_LIST, '[3, 2, 1]', []),
    Case('sort([2,1,3],(a,b)->return 1/0)',
         UNE_RK_ERROR, UNE_EK_ZERO_DIVISION, []),
...
def check_report(interface: str, case: Case, i: int):
        ...
        for line in report_status:
            if line.startswith('result_kind:'):
                ...
                if result_kind != case.result_kind:
                    print(
                        f'\33[0m[{i}/{CASES_LEN}] \33[31m\33[1m<{interface}> RSKIND (\33[7m{result_kind_have}\33[27m != {result_kind_want})\33[0m')
                    passed = False
                    if STOP_AT_FAIL:
                        return passed
                ...
                if alloc_count != 0:
                    print(
                        f'\33[0m[{i}/{CASES_LEN}] \33[31m\33[1m<{interface}> ALLOCS ({alloc_count})\33[0m')
                    passed = False
                    if STOP_AT_FAIL:
                        return passed
                ...
                if alert_count != 0:
                    print(
                        f'\33[0m[{i}/{CASES_LEN}] \33[31m\33[1m<{interface}> ALERTS ({alert_count})\33[0m')
                    passed = False
                    if STOP_AT_FAIL:
                        return passed
    ...
    with open(FILE_RETURN, mode='r', encoding='utf-8-sig') as report_return:
        ...
        if return_value != str(case.result_value):
            ...
            print(
                f'\33[0m[{i}/{CASES_LEN}] \33[31m\33[1m<{interface}> RETURN (\33[7m{return_value_have}\33[27m != {return_value_want})\33[0m')
            passed = False
...
for i, case in enumerate(cases):
    if i+1 < SKIP_UNTIL:
        continue
    une_no_output = '1>' + ('nul' if is_cmd_exe()
                            else '/dev/null') + f' 2>&1 {UNE}'
    une_with_output = UNE
